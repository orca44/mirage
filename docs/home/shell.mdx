...
<Tabs>
  ...
  <Tab title="CLI​" icon="terminal">
    ...
    mirage execute -w demo -c "env FOO=bar printenv FOO"
    ```
    The CLI doesn't have `--cwd` / `--env` flags, but bash subshell syntax `(cd ... && cmd)` and the `env` prefix give the same per-call isolation. Mutations inside the parens don't leak.
  </Tab>
</Tabs>
